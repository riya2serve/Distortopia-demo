...
@main.command(short_help="Introduce random SNPs/indels into a FASTA")
@click.option("--input",  "input_fa",  required=True, help="Reference FASTA file (.fa/.fna/.fasta)")
@click.option("--output", "output_fa", required=True, help="Output mutated FASTA file")
@click.option("--snp-rate",   type=float, default=0.001, show_default=True, help="Per-base SNP probability")
@click.option("--indel-rate", type=float, default=0.0,   show_default=True, help="Per-base indel probability")
@click.option("--seed",       type=int,   default=42,    show_default=True, help="Random seed for reproducibility")
def mutate(input_fa, output_fa, snp_rate, indel_rate, seed):
    """Introduce random SNPs and indels into a reference FASTA."""
    from .mutate import mutate_fasta
    mutate_fasta(input_fa, output_fa, snp_rate=snp_rate, indel_rate=indel_rate, seed=seed)
    click.echo(f"Wrote mutated FASTA → {output_fa}")
