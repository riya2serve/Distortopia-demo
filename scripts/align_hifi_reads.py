...
def run_pipe_to_file(p1, p2, outfile):
    ...
    rc = p_sort.wait()
    if rc != 0: raise RuntimeError("samtools sort failed")
...
def parse_flagstat(txt: str) -> dict:
    ...
    for line in txt.splitlines():
        ls = line.strip()
        if " in total" in ls and d["total"] is None: d["total"] = int(ls.split()[0])
        elif " mapped (" in ls and "primary" not in ls and d["mapped"] is None: d["mapped"] = int(ls.split()[0])
        elif "primary mapped" in ls and d["primary_mapped"] is None: d["primary_mapped"] = int(ls.split()[0])
    return d

def auto_out_bam(reads_path: Path, ref_path: Path, outdir: Path) -> Path:
    rname = reads_path.name
    if rname.endswith(".fq.gz"): r = rname[:-6]
    elif rname.endswith(".fastq.gz"): r = rname[:-9]
    else: r = reads_path.stem
    ref = ref_path.stem
...
def ensure_fasta_index(ref_fa: Path):
    ...
    if not fai.exists(): run_cmd_text(["samtools", "faidx", str(ref_fa)])

def ensure_bam_index(bam_p: Path):
    bai = Path(str(bam_p) + ".bai")
    if not bai.exists(): run_cmd_text(["samtools", "index", str(bam_p)])
...
def run(ref1: Path, ref2: Path, reads1: Path, reads2: Path, *,
        threads: int, outdir: Path, index_bam: bool, call_variants: bool,
        ploidy_mode: str, normalize_vcf: bool, require_pass: bool,
        mapq_min: int, baseq_min: int, qual_min: int, show_flagstat: bool=False) -> pd.DataFrame:
    for tool in ["minimap2","samtools"]:
        if not check_tool(tool): raise RuntimeError(f"`{tool}` not found")
    ...
    for reads_p, ref_p, bam_p in jobs:
        run_pipe_to_file(["minimap2","-t",str(int(threads)),"-ax","map-hifi", str(ref_p), str(reads_p)],
                         ["samtools","sort"], str(bam_p))
        if index_bam: run_cmd_text(["samtools","index",str(bam_p)])
        fs_text = run_cmd_text(["samtools","flagstat",str(bam_p)])
        fs = parse_flagstat(fs_text); total = fs.get("total") or 0; primary = fs.get("primary_mapped") or 0
        row = {"reads": reads_p.name, "ref": ref_p.name, "bam": str(bam_p),
               "pct_mapped_primary": round((primary/total*100) if total>0 else 0.0, 2)}
        if show_flagstat: row["flagstat"] = fs_text
        if call_variants:
...
def streamlit_panel(state):
    ...
    index_bam = st.checkbox("Index BAMs", True); show_flagstat = st.checkbox("Include flagstat text", False)
    ...
    if st.button("Run align+call", type="primary"):
        try:
            ...
            st.subheader("Per-pair summary"); st.dataframe(df, use_container_width=True)
